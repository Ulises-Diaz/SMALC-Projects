...
class BlocksGroup(pygame.sprite.OrderedUpdates):
    ...
    def _check_line_completion(self): 
        for i, row in enumerate(self.grid[::-1]):
            if all(row):
                self.score += 5
            self.lines_completed += 1  # Incrementa las líneas completadas
            ...
            self.update_grid()
            self._check_line_completion()  # Revisa si hay más líneas para completar
...
def main():
    ...
    while run:
        ...
        lines_text = font.render(
            f"Lines: {blocks.lines}", True, (255, 255, 255), bgcolor)
        score_text = font.render(
            f"Score: {blocks.score}", True, (255, 255, 255), bgcolor)
        draw_centered_surface(screen, score_text, 270)
        draw_centered_surface(screen, lines_text, 300)
        if game_over:
            draw_centered_surface(screen, game_over_text, 0)
        # Update.
        pygame.display.flip()
